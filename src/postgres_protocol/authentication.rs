...
impl AuthKind {
    pub fn parse(payload: &[u8]) -> Result<Self, DbError> {
        if payload.len() < 4 {
            return Err(DbError::new(String::from("Invalid authentication message")));
        }
        ...
        match auth_type {
            ...
            5 => {
                ...
                let salt = payload[4..8].try_into().map_err(|_| {
                    DbError::new(String::from("Invalid MD5 authentication payload"))
                })?;
    ...
    fn parse_sasl_mechanisms(payload: &[u8]) -> Result<Vec<String>, DbError> {
        ...
        for i in 0..payload.len() {
            if payload[i] == 0 {
                ...
                let mechanism = std::str::from_utf8(&payload[start..i])
                    .map_err(|_| DbError::new(String::from("Invalid UTF-8 in Sasl mechanism")))?;
        ...
        if mechanisms.is_empty() {
            return Err(DbError::new(String::from("No Sasl mechanisms found")));
        }
    ...
    fn parse_sasl_continue(payload: &[u8]) -> Result<String, DbError> {
        let message = std::str::from_utf8(payload)
            .map_err(|_| DbError::new(String::from("Invalid UTF-8 in Sasl continue message")))?;
    ...
    fn parse_sasl_final(payload: &[u8]) -> Result<String, DbError> {
        let message = std::str::from_utf8(payload)
            .map_err(|_| DbError::new(String::from("Invalid UTF-8 in Sasl final message")))?;
