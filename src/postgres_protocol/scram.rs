...
impl ScramClient {
    ...
    pub fn final_message(&mut self) -> Result<String, DbError> {
        ...
        Ok(format!("c=biws,r={},p={}", server_nonce, client_proof))
    }
    ...
    fn calculate_client_proof(&self) -> Result<String, DbError> {
        ...
        let salt = general_purpose::STANDARD
            .decode(salt)
            .map_err(|_| DbError::new(String::from("Invalid SCRAM salt")))?;
        ...
        let mut mac = HmacSha256::new_from_slice(&salted_password)
            .map_err(|_| DbError::new(String::from("Failed to create HMAC")))?;
        ...
        Ok(general_purpose::STANDARD.encode(client_proof))
    }
    ...
    pub fn handle_server_first(&mut self, message: &str) -> Result<(), DbError> {
        ...
        for part in message.split(',') {
            let (key, value) = part
                .split_once('=')
                .ok_or_else(|| DbError::new(String::from("Invalid SCRAM server-first-message")))?;
            ...
            match key {
                ...
                "i" => {
                    iterations = Some(value.parse::<u32>().map_err(|_| {
                        DbError::new(String::from("Invalid SCRAM iteration count"))
                    })?);
                }
                _ => {}
            }
        ...
        let server_nonce =
            server_nonce.ok_or_else(|| DbError::new(String::from("Missing server nonce")))?;

        let salt = salt.ok_or_else(|| DbError::new(String::from("Missing SCRAM salt")))?;

        let iterations =
            iterations.ok_or_else(|| DbError::new(String::from("Missing SCRAM iterations")))?;
    ...
    fn auth_message(&self) -> Result<String, DbError> {
        ...
        let server_first_message = self
            ...
            .as_ref()
            .ok_or_else(|| DbError::new(String::from("Missing server first message")))?;
    ...
    pub fn handle_server_final(&mut self, message: &str) -> Result<(), DbError> {
        let (key, value) = message
            .split_once('=')
            .ok_or_else(|| DbError::new(String::from("Invalid SCRAM server-final-message")))?;

        match key {
            "v" => {
                let received_signature = general_purpose::STANDARD
                    .decode(value)
                    .map_err(|_| DbError::new(String::from("Invalid SCRAM server signature")))?;
